...
This skill exists because "keep docs updated" is a standing rule that is easy to skip when it is unclear that a session has ended. Invoking this skill makes the step explicit and repeatable instead of waiting for the user to remember to ask.
...
2. Decide where each fact belongs.
   ...
   - Cross-project agent preferences go in the global `AGENTS.md`, but ask permission before editing that file.
